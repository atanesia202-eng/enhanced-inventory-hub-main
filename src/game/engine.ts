...
export function switchLocation(state: GameState, location: 0 | 1 | 2 | 3 | 4 | 5) {
  ...
  if (location === 0) {
    ...
    // Switch to field (map 1 - with monsters)
    state.platforms = MAP_PLATFORMS.map((p) => ({ ...p, pos: { ...p.pos } }));
    state.collectibles = MAP_COLLECTIBLES.map((c) => ({ ...c, pos: { ...c.pos } }));
    state.player.pos = { x: 180, y: 550 }; // Position at X:180 Y:550 in field
    state.worldWidth = WORLD_WIDTH;
    state.worldHeight = WORLD_HEIGHT;
    
    // Spawn enemies for field
    state.enemies = [];
    for (let i = 0; i < 15; i++) {
...
export function update(state: GameState) {
  ...
  for (const enemy of state.enemies) {
    ...
    if (enemy.state === 'hurt') {
    ...
    } else if (dist < 50 && enemy.stateTimer % ENEMY_ATTACK_INTERVAL_FRAMES === 0) {
      enemy.state = 'attack';
      enemy.stateTimer = 0;
      if (collides(enemy, player) && player.state !== 'hurt') {
      ...
      enemy.state = 'walk';
      const speed = enemy.isBoss ? 0.8 : 0.5;
      if (dist > 30) {
    ...
    for (const plat of state.platforms) {
      if (enemy.vel.vy >= 0 &&
        enemy.pos.x + enemy.width > plat.pos.x &&
        enemy.pos.x < plat.pos.x + plat.width &&
  ...
  state.enemies = state.enemies.filter(e => !(e.state === 'dead' && e.stateTimer > 30));

  // Spawn new enemies (only in field/location 1)
  state.spawnTimer--;

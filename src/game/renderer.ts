...
function drawNinja(ctx: CanvasRenderingContext2D, char: Character, cam: Position, isPlayer: boolean) {
  ...
  const bounce = char.state === 'walk' ? Math.sin(char.stateTimer * 0.3) * 3 : 0;
...
function drawEnemy(ctx: CanvasRenderingContext2D, enemy: Enemy, cam: Position) {
  ...
  const bounce = enemy.state === 'walk' ? Math.sin(enemy.stateTimer * 0.25) * 2 : 0;
  const hurtShake = enemy.state === 'hurt' ? (Math.random() - 0.5) * 4 : 0;
  ctx.translate(hurtShake, bounce);

  const w = enemy.width;
  const h = enemy.height;

  // Try to draw custom enemy image
  ...
  if (enemy.state !== 'dead') {
    const barW = enemy.isBoss ? 60 : 40;
    const barH = enemy.isBoss ? 6 : 4;
    const barX = x + enemy.width / 2 - barW / 2;
    const barY = y - 10;
    ctx.fillStyle = '#1a202c';
    ctx.fillRect(barX, barY, barW, barH);
    ctx.fillStyle = enemy.isBoss ? '#ff4444' : '#ef4444';
    ctx.fillRect(barX, barY, barW * (enemy.hp / enemy.maxHp), barH);
    if (enemy.isBoss) {
      ctx.fillStyle = '#ffd700';
      ctx.font = 'bold 8px "Press Start 2P"';
      ctx.textAlign = 'center';
      ctx.fillText('BOSS', x + enemy.width / 2, barY - 4);
    }
  }
}
...
function drawMinimap(ctx: CanvasRenderingContext2D, state: GameState) {
  ...
  ctx.fillText(`X:${coordX} Y:${coordY}`, minimapX - 50, minimapY - 8);
